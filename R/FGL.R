  ...
  admm.iters_copula = function(S, lambda1,lambda2,rho=1,rho.increment=1,weights,penalize.diagonal,maxiter = 1000,tol=1e-5)
  {
    ...
    while((iter==0) || (iter<maxiter && diff_value > tol))
    {
      ...
      if(FALSE)
      {
        ...
        message(paste("crit=",crit(theta,S,n=rep(1,K),lam1,lam2,penalize.diagonal=penalize.diagonal)))
        message(paste("crit=",crit(Z,S,n=rep(1,K),lam1,lam2,penalize.diagonal=penalize.diagonal)))
      }
      # if(FALSE)
      # {
